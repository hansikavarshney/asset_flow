...
const defaultSeed = {
  ...
  notifications: [
    { id: 'ntf-mock-1', userId: 'usr-4', title: 'Asset Allocated', message: 'Laptop AF-0014 assigned to Priya shah', type: 'success', category: 'approval', read: false, timestamp: new Date(Date.now() - 2 * 60 * 1000).toISOString() },
    { id: 'ntf-mock-2', userId: 'usr-2', title: 'Maintenance Approved', message: 'Maintenance request AF-0055 approved', type: 'success', category: 'approval', read: false, timestamp: new Date(Date.now() - 18 * 60 * 1000).toISOString() },
    { id: 'ntf-mock-3', userId: 'usr-4', title: 'Booking Confirmed', message: 'Booking confirmed: Room B2 : 2:00 to 3:00 PM', type: 'info', category: 'booking', read: false, timestamp: new Date(Date.now() - 60 * 60 * 1000).toISOString() },
    { id: 'ntf-mock-4', userId: 'usr-2', title: 'Transfer Approved', message: 'Transfer approved: AF-0033 to facilities dept', type: 'success', category: 'approval', read: false, timestamp: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString() },
    { id: 'ntf-mock-5', userId: 'usr-3', title: 'Overdue Return', message: 'Overdue return: AF-0021 was due 3 days ago', type: 'danger', category: 'alert', read: false, timestamp: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString() },
    { id: 'ntf-mock-6', userId: 'usr-2', title: 'Audit Discrepancy', message: 'audit discrepancy flagged: AF-0088 damaged', type: 'danger', category: 'alert', read: false, timestamp: new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString() }
  ]
};
...
const AssetFlowDB = {
  ...
  notify(userId, title, message, type = 'info', category = null) {
    if (!category) {
      const titleLower = title.toLowerCase();
      const msgLower = message.toLowerCase();
      if (titleLower.includes('booking') || msgLower.includes('booking') || titleLower.includes('reservation') || msgLower.includes('reservation')) {
        category = 'booking';
      } else if (titleLower.includes('approved') || msgLower.includes('approved') || 
                 titleLower.includes('allocated') || msgLower.includes('allocated') || 
                 titleLower.includes('transfer') || msgLower.includes('transfer') || 
                 titleLower.includes('request') || msgLower.includes('request') || 
                 titleLower.includes('repair') || msgLower.includes('repair') || 
                 titleLower.includes('completed') || msgLower.includes('completed') || 
                 titleLower.includes('rejected') || msgLower.includes('rejected')) {
        category = 'approval';
      } else {
        category = 'alert';
      }
  ...
  getNotificationsForUser(user) {
    const all = this.getAll('notifications') || [];
    if (!user) return [];
    ...
    if (user.role === 'Department Head') {
      ...
      const deptUsers = users.filter(u => u.departmentId === user.departmentId).map(u => u.id);
      return all.filter(n => n.userId === user.id || deptUsers.includes(n.userId));
    }
