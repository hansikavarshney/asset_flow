...
function renderAuthPage() {
  currentRoute = 'login';
  // Reset landing body class to normal app view
  document.body.className = '';
  const root = document.getElementById('app-root');
  root.innerHTML = `
    <div class="auth-container" style="position: relative; overflow: hidden; background: var(--bg-secondary);">
      <!-- Soft blur background blobs for Stripe/Linear styling -->
      <div style="position: absolute; width: 350px; height: 350px; border-radius: 50%; filter: blur(100px); background: rgba(37, 99, 235, 0.12); top: -50px; left: -50px; pointer-events: none;"></div>
      <div style="position: absolute; width: 450px; height: 450px; border-radius: 50%; filter: blur(120px); background: rgba(79, 70, 229, 0.12); bottom: -100px; right: -100px; pointer-events: none;"></div>
      
      <!-- Back to Home floating pill button -->
      <button onclick="renderLandingPage()" style="position: absolute; top: 1.5rem; left: 1.5rem; font-family: var(--font-sans); font-size: 0.875rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem; border: 1px solid var(--border-color); background: var(--bg-primary); color: var(--text-primary); padding: 0.5rem 1.25rem; border-radius: 9999px; cursor: pointer; box-shadow: var(--shadow-sm); z-index: 10; transition: all 0.2s ease;">
        <i data-lucide="arrow-left" style="width: 16px; height: 16px;"></i> Back to Home
      </button>

      <div class="auth-card" id="auth-card-body" style="border-radius: var(--radius-lg); border: 1px solid var(--border-color); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); background: var(--glass-bg); z-index: 2;">
...
function renderAppShell() {
  document.body.className = ''; // Reset body class from landing page
  const root = document.getElementById('app-root');
...
function getSidebarNavigation() {
  ...
  let html = `
    ...
    <li>
      <a class="nav-item ${currentRoute === 'notifications' ? 'active' : ''}" onclick="navigateTo('notifications')" style="position:relative;">
        <i data-lucide="bell"></i>
        <span>Notifications</span>
        ${unreadCount > 0 ? `<span style="margin-left:auto; background:var(--danger); color:#fff; font-size:0.65rem; font-weight:700; border-radius:9999px; padding:0.1rem 0.4rem; min-width:18px; text-align:center;">${unreadCount}</span>` : ''}
      </a>
    </li>
  `;
    ...
    setTimeout(() => {
      ...
      switch(route) {
        ...
        case 'reports':
          ...
          break;
        case 'logs':
          if (currentUser.role === 'Admin') renderActivityLogs(container);
          else navigateTo('dashboard');
          break;
        case 'notifications':
...
function getUnreadNotificationCount() {
  ...
  return AssetFlowDB.getNotificationsForUser(currentUser).filter(n => !n.read).length;
}
